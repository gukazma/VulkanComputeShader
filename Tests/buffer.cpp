...
Buffer::Buffer(vk::PhysicalDevice phyDevice, vk::Device device, vk::BufferUsageFlags usage,
               size_t size, vk::MemoryPropertyFlags memProperty)
    : device_(device)
    , phyDevice_(phyDevice)
{
    this->size = size;
    vk::BufferCreateInfo createInfo;
    createInfo.setUsage(usage).setSize(size).setSharingMode(vk::SharingMode::eExclusive);
